...
multipleDeclSplitter=$(command -v multipleDeclSplitter 2>/dev/null || echo "$curPath/../../../build/multipleDeclSplitter")
...
for file in $testsPath/*.cpp; do
    ...
    if [ -f "$file" ]; then
        ...
        $multipleDeclSplitter $file $arguments > "$folderResultPath/$fileName" 2> /dev/null
