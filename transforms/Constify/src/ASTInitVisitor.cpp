    ...
    if ((intype->isPointerType() || intype->isReferenceType())) {
      ...
      if (valueInit(v)) {
        ...
        if (initDeclArg != nullptr) {
          ...
          // The value is referenced, so if it's unconst, we have to unconst
          // what refers to it
...
bool ASTInitVisitor::VisitCallExpr(CallExpr *ce) {
  ...
  if (fdec != nullptr &&
    ...
    if (isa<CXXMemberCallExpr>(ce)) {
      ...
      const_arg *methDeclArg =
          SymT.getInnerConstArg(memberCall->getMethodDecl());
...
bool ASTInitVisitor::VisitBinaryOperator(BinaryOperator *bop) {
  ...
  // Assignement (outside of declarations) might add a dependency between a
  // value and a pointer
