...
bool ASTHeapifyVisitor::subVisitCompoundStmt(CompoundStmt *coSt) {
  bool deleteEnd = true; // If we have to delete at the end of the scope (so if
                         // there is no return)
  // bool curState=variableHeap.found; //If variable is found, we're in a
  // subLoop, so no delete (unless there is a return)
  ...
  for (CompoundStmt::body_iterator b = coSt->body_begin(), e = coSt->body_end();
    ...
    if (isa<ReturnStmt>(st)) {
      ...
      deleteEnd = false; // No need to delete at the end of the scope if there
                         // is a return (and so a delete)
  ...
  if (deleteEnd) {
    TheRewriter.InsertTextAfter(coSt->getEndLoc(),
                                createDeleteSegment(currentVarsInScope));
  }
  // We remove variables declared in the scope at the end of it (since they no
  // longer exist)
...
void ASTHeapifyVisitor::subVisitIfStmt(IfStmt *ifSt) {
  ...
  // When there is a declaration in the If, we add a section from its beginning
  // to its end (after the else)
...
void ASTHeapifyVisitor::handleDeclStmt(
  ...
  TheRewriter.ReplaceText(
      SourceRange(decStmt->getBeginLoc(), decStmt->getEndLoc()), SSprint.str());
  // True when variable was found in the current scope, so we have to add a
