...
// Mostly here to reset the counter for variables, we assume that number will
// not overflow
...
void ASTUnstackVisitor::subVisitDeclStmt(DeclStmt *declSt) {
  ...
  // Vector containing all expressions that might need to be transformed by the
  // transformation
...
// instructionBegin
void ASTUnstackVisitor::unstackTransformCallExpr(
...
void ASTUnstackVisitor::findTopCallsInExpr(
    Expr *ex, std::vector<CallExpr *> &callExprVect) {
  if (ex == NULL) {
    return;
  }
...
void ASTUnstackVisitor::findAllCallExpr(CallExpr *calExp,
                                        std::vector<CallExpr *> &vectCallExpr) {
  // We look for the CallExpr in each Argument
  for (auto beg = calExp->arg_begin(), end = calExp->arg_end(); beg != end;
       beg++) {
    Expr *arg = (*beg);
...
std::string
ASTUnstackVisitor::createCallArgString(Expr *argExpr,
                                       std::queue<int> &tempVarQueue) {
...
std::string ASTUnstackVisitor::createTempVarString(
...
void ASTUnstackVisitor::transfoInstruction(
    std::stack<Expr *> &exprStack, const SourceLocation &instructionBeginLoc) {
  while (!exprStack.empty()) {
    tranfoExpr(exprStack.top(), instructionBeginLoc);
    exprStack.pop();
  }
}
void ASTUnstackVisitor::tranfoExpr(Expr *ex,
