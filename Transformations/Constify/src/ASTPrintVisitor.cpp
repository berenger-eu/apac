...
bool ASTPrintVisitor::VisitDeclStmt(DeclStmt *declStatement) {
  if (TheRewriter.getSourceMgr().isInSystemHeader(
          declStatement->getBeginLoc())) {
    return true;
  }
  ...
  // Multiple decl
    ...
    TheRewriter.RemoveText(
        SourceRange(declStatement->getBeginLoc(), declStatement->getEndLoc()));
...
void ASTPrintVisitor::PrepareRewriteVarDecl(VarDecl *v,
...
void ASTPrintVisitor::rewriteSingleDecl(VarDecl *vd) {
  ...
  if (SymT.getHashTableValue(vd)->is_const && vd->getInit() != NULL) {
    ...
    TheRewriter.ReplaceText(
        SourceRange(vd->getBeginLoc(), vd->getTypeSpecEndLoc()), result);
  }
}
...
void addConstToVar(ValueDecl *valD) {
  QualType newQualType =
      addConstToQualType(valD->getType(), valD->getASTContext());
...
QualType addConstToQualType(QualType qt, ASTContext &aContext) {
  ...
  if (innerType == NULL)
    ...
    qt.addConst();
    llvm::outs() << "Adding const to the following type : " << qt.getAsString()
                 << " might not work\n";
    qt->dump();
    innerType->dump();
  }
...
QualType addConstToReferenceType(QualType qt, ASTContext &aContext) {
  QualType refInnerType =
      addConstToQualType(qt.getNonReferenceType(), aContext);
