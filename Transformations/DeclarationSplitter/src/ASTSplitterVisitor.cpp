...
bool ASTSplitterVisitor::VisitDeclStmt(DeclStmt* declSt){
    ...
    {
        TheRewriter.ReplaceText(SourceRange(declSt->getBeginLoc(),declSt->getEndLoc()),SSprintFinal.str());
    }
...
void ASTSplitterVisitor::stringVarDecl(const VarDecl& v,std::stringstream& SSprintDecl, std::stringstream& SSprintInit){
    ...
    if(isReferenceQualType(v.getType()))
    {
        ...
        || !isa<DeclRefExpr>(init))
        {
            ...
            SSprintDecl<<"std::reference_wrapper<"<<qType.getAsString()<<"> "<<v.getNameAsString()
            //= invalid_ref<unqualified type>();
            <<" = invalid_ref<"<<qType.getUnqualifiedType().getAsString()<<">();\n";
            SSprintDecl<<v.getType().getAsString()<<" __refT_"<<v.getNameAsString()<<"="<<getInitString(v)<<";\n";
    ...
    else if(v.getType().isConstQualified()&&v.getInit())
    {
        ...
        //Creates std::reference_wrapper<type>
        SSprintDecl<<"std::reference_wrapper<"<<v.getType().getAsString()<<"> "<<v.getNameAsString()
        //= invalid_ref<unqualified type>();
        <<" = invalid_ref<"<<v.getType().getUnqualifiedType().getAsString()<<">();\n";
        SSprintDecl<<qType.getAsString()<<" __refT_"<<v.getNameAsString()<<"="<<getInitString(v)<<";\n";
        SSprintInit<<v.getNameAsString()<<"= __refT_"<<v.getNameAsString()<<";\n";
