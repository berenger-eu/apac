...
public:
  MyASTConsumer(Rewriter &R) : VisitorMainPara(R), TheRewriter(R) {}

  // Parse all the file
  virtual void HandleTranslationUnit(ASTContext &Ctx) {
    VisitorMainPara.TraverseAST(Ctx);
    VisitorMainPara.addParaZone();
  }
...
bool MainParallelHandler::run(int argc, const char **argv) {
  if (argc < 2) {
...
int main(int argc, const char **argv) {
  return MainParallelHandler::run(argc, argv);
}
